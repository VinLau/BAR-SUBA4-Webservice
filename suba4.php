...
# adding by the score param (value) to the scores array
# Note that scores is passed by reference not value so that we can directly modify the 
# array to be used in future iterations (to keep memory of previous scores!)
...
function main(){
	...
	if ($result = mysqli_query($con, $sql)) {
		...
		foreach($decodedData['AGI_IDs'] as $gene) {
			...
			); #need to use this format to allow for empty objet {} printing	
		}
		...
		while( $row = mysqli_fetch_assoc($result) ) {
			...
			foreach($expColNames as $expCol){
				...
				if ( $expColLocs !== NULL ) { # check if field value is NOT NULL
					processLocalizations($expColLocs, $resJSON[$id]['data'], $expValue);
					$resJSON[$id]['includes_experimental'] = "yes";
				}
			}

			# The "data" field of the AGI in the $resJSON is an ass array i.e. ( "nucleus" => "10", "cytosol" => "2")
			# Let us sort this so the API user does not need to sort it (i.e. mainly for AIV2 =) )
			# That also means we need to rechange the JSON form to a 2D array [["nucleus", 10] ["cytosol", 20]]
			arsort($resJSON[$id]['data']);
			$dataSortedArr = [];
			foreach($resJSON[$id]['data'] as $key => $value) {
				$dataSortedArr[] = [$key => $value];
			}
			$resJSON[$id]['data'] = $dataSortedArr;
		}
